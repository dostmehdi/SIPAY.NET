...
namespace SipayCoreCore
{
    public class SipayPaymentService
    {
        ...
        public static SipaySmartPaymentResponse SmartPay(SipaySmartPaymentRequest request, Settings settings, string token)
        {
            var header = new Dictionary<string, string>();
            header.Add("Authorization", "Bearer " + token);

            SipaySmartPaymentResponse response = PostDataAsync<SipaySmartPaymentResponse, SipaySmartPaymentRequest>(settings.BaseUrl + "/api/paySmart2D", request, header);
